...
exists, not here. That keeps the arithmetic testable with no GPS machinery.
  ...
  - A stop before its start is a hard REJECT (it would store negative hours).
...
def start(d: db.Database, when: datetime, *, session_id: int | None = None) -> EngineResult:
    ...
    with d.conn:
        cur = d.conn.execute(
            "INSERT INTO engine_run(session_id, started_utc, method, open) "
            "VALUES (?, ?, 'paired', 1)",
            (session_id, db.to_iso_utc(when)),
        )
    return EngineResult(run_id=cur.lastrowid, duration_min=None, warnings=warnings)


def stop(d: db.Database, when: datetime) -> EngineResult:
    ...
    if when <= started:
        raise EngineError("stop time is not after the start time (would be a negative run)")
    duration = (when - started).total_seconds() / 60.0
    warnings = _overlap_warnings(d, started, when, exclude_id=run["id"])
    with d.conn:
        d.conn.execute(
            "UPDATE engine_run SET stopped_utc = ?, duration_min = ?, open = 0 WHERE id = ?",
            (db.to_iso_utc(when), duration, run["id"]),
        )
...
def add_completed(
    ...
    *,
    ...
    if has_times:
        ...
        if stopped <= started:
            raise EngineError("stop time is not after the start time")
        ...
        if duration_min <= 0:
            raise EngineError("duration must be positive")
    ...
    with d.conn:
        cur = d.conn.execute(
            "INSERT INTO engine_run(session_id, started_utc, stopped_utc, duration_min, method, open) "
            "VALUES (?, ?, ?, ?, ?, 0)",
            (session_id, started_iso, stopped_iso, duration, method),
        )
